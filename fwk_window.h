...
char* window_stats() {
    ...
    static char buf[192];
    snprintf(buf, 192, "%s | boot %.2fs | %5.2ffps (%.2fms)", title, boot_time, fps, (now - prev_frame) * 1000.f);
...
void window_visible(int visible) {
    ...
    //@fixme: call glfwpollevents in linux to force flush visiblity changes that would happen in next frame otherwise
}
int window_has_visible() {
