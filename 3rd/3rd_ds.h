...
#endif
    static THREAD int cur = 0, len = STACK_ALLOC; //printf("string stack %d/%d\n", cur, STACK_ALLOC);
...
uint64_t hash_str(const char* str); // non-const arg for C++
...
uint64_t hash_str(const char* str) { // non-const arg for C++
    // hash(0), mul(131) faster than fnv1a, a few more collisions though.
